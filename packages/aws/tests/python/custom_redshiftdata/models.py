...
class Statement(BaseModel, ManagedState):
    def __init__(
        self,
        cluster_identifier,
        database,
        ...
        super().__init__(
            model_name="redshift-data::statement", transitions=[("STARTED", "FINISHED")]
        )
    ...
    def __iter__(self):
        ...
        yield "HasResultSet", self.status == "FINISHED"
        ...
        yield "ResultRows", -1 if self.status == "STARTED" else 3
        yield "ResultSize", -1 if self.status == "STARTED" else 256
...
class RedshiftDataAPIServiceBackend(BaseBackend):
    def __init__(self, region_name=None):
        state_manager.register_default_transition(
            ...
            transition={"progression": "time", "seconds": 5},
        )
