...
- **Security:** Bump the following dependencies across all package lock files:
  ...
  - ``PyJWT`` ≥2.13.0 (HIGH/MEDIUM/LOW — SSRF via JWK/JWKS, algorithm allow-list bypass,
    DoS; CVE-2026-48522 through CVE-2026-48526)
  - ``pypdf`` ≥6.13.3 (MEDIUM — missing stream length limits; supersedes CVE-2026-48155
    fix from :pr:`1334`)
  - ``msgpack`` ≥1.2.1 (HIGH — out-of-bounds read/crash on Unpacker reuse)
  - ``zeep`` ≥4.3.3 (MEDIUM — SSRF)
  - ``pytest`` ≥9.0.3 and ``Pygments`` ≥2.20.0 in the root dev lock (:pr:`1334`)

  Direct dependency floors tightened in ``rpaframework`` (``cryptography``, ``PyJWT``)
  and ``rpaframework-pdf`` (``pypdf``); ``msgpack`` pinned via override in
...
- Security: Enable GitHub CodeQL scanning and add Dependabot (:pr:`1228`).
- Packaging: Migrate from Poetry lockfiles to uv; update workflows; bump ``h11>=0.16.0`` in root dependencies.
- Robot test stability improvements across multiple suites.

- ``rpaframework`` **31.0.0**
- ``rpaframework-aws`` **7.0.0**
- ``rpaframework-assistant`` **5.0.0**
- ``rpaframework-google`` **11.0.0**
- ``rpaframework-hubspot`` **3.0.0**
- ``rpaframework-openai`` **4.0.0**
...
- Library **RPA.Robocorp.Vault** (:pr:`1129`): Improve error message when trying to
  connect to Control Room Vault without valid connection details.
- Library **RPA.SAP** (:pr:`1130`): Add keywords for pressing F1 or F4 on textfield
  and gridview elements. Thanks to `Enrico Franzelli <https://github.com/wisenrico>`_
...
- Library **RPA.Outlook.Application** (:pr:`1127`): Fix how keyword ``Move Emails`` works.
